...
SKIP_DIRS = {'.git', 'venv', '__pycache__', '.backups', 'node_modules',
             'logs', 'update', '.tracker.pid', 'audits', 'graphify-out'}
...
echo "  2. Package (tracked-files-only): bash scripts/package-release.sh"
echo "     → ../aerodrome-packages/aerodrome-v${NEW_VERSION}.zip + .sha256"
echo "  3. Stage for preview (maintainer box): bash ~/git/scripts/stage-aerodrome.sh"
echo "  4. After the preview is verified, publish:"
echo "     bash scripts/publish-release.sh ${NEW_VERSION} --target <zip-tree-commit>"
echo "     (Derives tag/title/assets/notes and runs the verify loop. Do NOT"
echo "      hand-run 'gh release create'.)"
echo ""
