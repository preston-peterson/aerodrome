...
### Added
- **Server-side backup and restore for databases too large for the browser, plus streaming uploads everywhere.** Surfaced during a real production restore: a 2.18 GB backup was rejected by `/api/backup/import` because of a hardcoded 2 GB cap, and even with the cap raised the underlying mechanism — buffering the entire upload into Python memory before processing it — wouldn't scale. The 2 GB cap was a sanity check from a time when "real Aerodrome users with year-plus retention" wasn't a category that existed yet. v3.4.0 rebuilds the backup/restore architecture around the working assumption that databases can grow into the hundreds of GB and the browser is not always a viable transport.

  **Two new code paths, one zip format.** The existing browser-mediated flow (`Download full backup` button → upload to `Restore from backup`) keeps working for small databases — same zip format, same restore semantics, same `.pre-restore` safety snapshots. Added next to it: a server-side flow that writes the same zip directly to `<install_dir>/.backups/aerodrome-backup-<timestamp>.zip` on the host, with a sidecar `.sha256` file, and provides an in-UI list with restore/copy-scp-command/delete actions. The two paths produce bit-identical backup files — you can `scp` a server-side backup onto a different host and use the browser-restore flow against it, or vice versa. One mental model, two transports.

  **The streaming-uploads fix is independent and applies to both paths.** Before v3.4.0, `POST /api/backup/import` called `await file.read()` to slurp the entire upload into a Python `bytes` object, then wrapped it in a `BytesIO` for `zipfile.ZipFile` to read from. A 2 GB upload meant 2 GB of process RSS just to *check* it was valid. On a Pi-class host this could OOM the process before the size check fired. v3.4.0 streams the upload in 1 MB chunks to `<install_dir>/.backups/.uploads/upload-<ts>-<pid>.zip`, then processes the zip from disk. Peak memory is the chunk size, not the upload size. The 2 GB hardcoded cap is gone; replaced with an honest disk-space pre-check (need bytes_written + current_db_size + 256 MB headroom, returns HTTP 507 with a clear message if insufficient). The export side got the same streaming treatment — the zip is built on a temp file under `.backups/` and streamed out in 1 MB chunks rather than living in a 100 GB `BytesIO`.

  **Out-of-band transfer pattern documented.** A 50 GB backup zip doesn't belong in a browser session. The server-side flow's "Copy scp command" button templates the right invocation from the user's view of the host:

  ```
  scp user@aerodrome-host:/opt/aerodrome/.backups/aerodrome-backup-20260512-150658.zip ./
  ```

  Users edit the username if needed and run from their laptop or backup destination. For automated off-host backup, the recommended pattern in `docs/INSTALL.md` is a separate machine's cron running rsync against the `.backups/` directory daily — works at any database scale and decouples backup retention from Aerodrome's awareness.

  **Five new endpoints in `server.py`:**
  - `POST /api/backup/create-server-side` — kicks off a backup, returns when complete with `{ok, path, filename, bytes, sha256, duration_seconds, manifest}`. Uses SQLite's online backup API so writes continue against the live DB during the snapshot. Disk-space pre-check rejects backups that would exhaust the host. Concurrency-locked via a module-level `_server_side_backup_state` dict — only one server-side backup at a time, second attempt returns HTTP 409 with current progress.
  - `GET /api/backup/create-server-side/progress` — poll endpoint for the running backup, returns `{running, phase, percent, bytes_written, path}`. Driven by a `progress_callback(phase, percent, bytes_written)` threaded through the new shared `_build_backup_zip()` helper so phase reporting is honest (`manifest` → `config` → `db_snapshot` → `db_zip` → `db_zipped` → `done`).
  - `GET /api/backup/list-server-side` — lists user-initiated server-side backups under `.backups/aerodrome-backup-*.zip` (NOT the auto-managed `.pre-restore.*` snapshots — those have their own endpoint). For each: filename, full path, bytes, mtime, sha256 (read from sidecar; computed once at backup time, not re-hashed on every list). Sorted newest-first.
  - `POST /api/backup/restore-from-path` — restores from a backup file already on disk. Body: `{path}`. Path must resolve inside `<install_dir>/.backups/` by default; admin who scp'd a file from a remote backup destination drops it there first. Disk-space pre-check matches the upload path. Calls the same `_restore_from_local_zip()` helper as the streaming-upload path, so restore semantics are identical regardless of transport. Source file is NOT deleted on success — the user explicitly chose to keep a backup on disk by using server-side restore.
  - `DELETE /api/backup/server-side/{filename}` — removes a named backup + its `.sha256` sidecar. Filename validation rejects path separators, `..`, anything not matching `aerodrome-backup-*.zip`. Resolved path must remain inside `.backups/`.

  **Shared backup-build helper.** The existing `/api/backup/export` and the new `/api/backup/create-server-side` both call `_build_backup_zip(install_dir, dest_path, progress_callback=None)`, which writes the zip incrementally to `dest_path` (a filesystem path string). The browser-export path passes a temp file under `.backups/` and streams it back to the client; the server-side path writes directly to the final destination. Same code, same zip layout, same SQLite snapshot dance — just different destination semantics. The only DB-sized buffer in either path is the SQLite `.backup` temp file, which gets removed after `zf.write` reads it.

  **Shared restore helper.** Both `/api/backup/import` and `/api/backup/restore-from-path` call `_restore_from_local_zip(zip_path, install_dir, source_label, cleanup_source_on_success)`, which reads the backup from a local filesystem path (no memory buffering, no upload assumptions). The `cleanup_source_on_success` parameter is True for uploads (the staged temp file is no longer needed once restore succeeds) and False for server-side paths (the user's backup file should survive past the restore). Restore semantics are otherwise identical: manifest validation, wrapper-folder detection, `.pre-restore.<timestamp>` safety snapshots, ntfy auto-install on `not_installed` state (the v3.3.1 fix), service restart at the end.

  **UI integration in `templates/config.html`.** New "Server-side backup" section between "Full backup" and "Restore safety snapshots". Three subsections inside it: "Create new server-side backup" with progress display, "Existing server-side backups" with the list + actions, "Restore from a path on disk" with a path input for the edge case where the user has a backup zip somewhere outside `.backups/` and wants to restore from it directly (must move it into `.backups/` first; the helpful error message in the API tells them so). The list polls `/api/backup/list-server-side` and shows each backup with filename, size in MB, mtime in local time, sha256 prefix, and three buttons: "Restore from this" (destructive-confirm modal then call `/api/backup/restore-from-path`), "Copy scp command" (templates from `window.location.hostname`), "Delete" (destructive-confirm then `DELETE /api/backup/server-side/<filename>`). The create button polls `/api/backup/create-server-side/progress` every second so users see live `phase (N%, X.X MB written)` updates during long backups.

  **Documentation in `docs/INSTALL.md`.** New "Server-side backup (v3.4.0)" subsection under "Backup and restore" with: when to use which flow (table by database size), the scp out-of-band recipe, the rsync-from-cron recipe for automated off-host backup, the destructive-confirm restore-from-path walkthrough. Honest about the limits: above ~50 GB, filesystem-level snapshots (ZFS, btrfs, LVM) or WAL shipping become the right pattern; v3.4.0's server-side flow is the building block but not the complete answer at that scale.

  **Why this is v3.4.0 rather than v3.3.2.** The streaming-uploads fix alone would be a patch — bug fix, internal-implementation, no new surface. The server-side backup flow is a new feature with five new endpoints, a new UI section, and a new operational pattern (out-of-band transfer). New capability across the user-visible surface is a minor bump per semver. Notably no breaking changes — the existing browser-mediated flow keeps working identically, and the zip format is unchanged.

  **Operational note:** Aerodrome installs whose database exceeds the old 2 GB cap (or is approaching it) should adopt the server-side flow as the default. Browser-mediated full backup keeps working but is increasingly impractical above ~2 GB. The "Copy scp command" button + a cron-rsync from a separate machine is what real-scale backup looks like for an Aerodrome that grows into the year-plus-retention regime. **730M-row scale (the worst-case bound roughly equivalent to 180 GB on disk) is the architecture target, not the validated target — that level of validation requires real-world data accumulation over weeks and is outside what any code review can cover.**

  **Scope:**
  - `server.py` — new `_build_backup_zip()` and `_restore_from_local_zip()` helpers (~150 lines), refactor of `backup_export` to use the helper via temp file + 1MB-chunk yield (~30 lines net), refactor of `backup_import` to stream uploads to disk + disk-space check + delegate to restore helper (~45 lines net), five new endpoints (~250 lines)
  - `templates/config.html` — new Server-side backup section (~75 lines HTML) + JS for create/list/restore-from-path/delete + scp-command-copy helper (~165 lines)
  - `docs/INSTALL.md` — new "Server-side backup (v3.4.0)" subsection (~60 lines) under existing Backup and restore section
  - No schema changes, no config-key changes, no sudoers changes. SUDOERS_VERSION unchanged at 4.

## [3.3.1] — 2026-05-12

### Fixed
- **Two fixes to keep ntfy from ending up in silent-failure states, plus polish.** Surfaced during the v3.3.0 prod migration (backup → uninstall → fresh install at /opt/aerodrome → restore): the restore correctly identified that the backup contained an Aerodrome-managed ntfy server but stashed the config to `ntfy-server.yml.from-backup.<timestamp>` and printed a "saved for manual review" message instead of actually re-installing ntfy. The user-visible result was a working dashboard but broken push notifications, with no clear pointer to the recovery step. v3.3.1 fixes this end-to-end and also tightens the related UX so users can't enable notifications in a state where they'd silently fail.

  **Fix #1 — auto-install ntfy during restore when the backup contains it.** Previous behavior: restore checked `install_status()` and only wrote the ntfy server config when state was already `aerodrome_managed`. Anything else (including `not_installed` on a fresh install) stashed the config to a `.from-backup` sidecar and left the user to figure out the recovery. New behavior: when the backup contains `ntfy/server.yml` AND current state is `not_installed`, the restore calls `install()` from `ntfy_installer.py` first (idempotent, same install path the in-UI button uses), then overwrites the freshly-written default config with the backup's config, then restarts ntfy. Same outcome as the "already aerodrome_managed" path, just with one extra silent step at the front. Other non-managed states (`external` — user installed ntfy themselves via apt/dnf; `system_managed` — distro shipped it) still stash the config without auto-installing, because we never take over a user's own ntfy install. Warning message in the restore summary tells the user what happened: *"Re-installed Aerodrome-managed ntfy server (was not present in the new install — restored from backup automatically)."*

  **Fix #2 — gate the "Notifications enabled" checkbox when local ntfy is required but missing.** Previous behavior: the Notifications tab let users check "Notifications enabled" regardless of whether the configured `notifications.url` was reachable. The most common silent-failure pattern: `notifications.url` points at `http://localhost:2586/...` but no local ntfy is installed yet. The checkbox saves fine, the UI looks correct, and every notification attempt POSTs into the void. New behavior: client-side, the checkbox disables and shows an inline warning (*"⚠ Local ntfy not installed. Notifications would silently fail with this URL. Install local ntfy below, then re-check this box."*) whenever the URL points at localhost/127.0.0.1 AND `install_status()` reports `not_installed` or `partial`. The check re-runs live when the user edits the URL field — switching the URL to `https://ntfy.sh/your-topic` immediately re-enables the checkbox. Server-side, the `PUT /api/config` endpoint runs the same check post-validation and rejects the save with a clear error message — defense in depth for raw API callers and stale browser tabs. Users with external ntfy URLs (ntfy.sh, custom servers, Tailscale tunnels) are unaffected; the gate only fires for the specific localhost-without-local-ntfy combination.

  **Polish items bundled along:**
  - **uninstall.sh step numbering.** Three `[N/5]` labels were left over from when uninstall had five steps; the script grew to six (the ntfy-cleanup step + the synthetic-feeder service cleanup) but the labels weren't renumbered. Fixed to `[1/6]` through `[6/6]`. Cosmetic but jarring — visible in every uninstall output.
  - **Python version-check error message.** When `python3 -c '...'` failed entirely (broken interpreter, stale bytecode, bad site hooks — surfaced during v3.3.0 Arch dogfood on a VM with a corrupted Python 3.14 install), the bootstrap printed "Python 3.10+ required; found Python 3.14.4" and pointed at the deadsnakes PPA — none of which helped. v3.3.1 distinguishes the two failure modes: if `python3 --version` works but the version is too old → keep the existing deadsnakes hint. If `python3 -c` fails outright → show the actual error output (which previously was suppressed with `2>/dev/null`), and offer a family-aware reinstall command (`sudo apt-get install --reinstall python3` / `sudo dnf reinstall python3` / `sudo pacman -S python` / `sudo zypper install --force python3`). The bootstrap detects the broken interpreter early, so the install never gets far enough to crash later in a more confusing way.

  **Why this matters operationally.** The pattern across all four items is the same: a state where the system *looks* correct but functionally isn't, and where the user has no clear pointer to the recovery step. The fix in each case is either auto-recovery (ntfy on restore), prevention (gate the checkbox), or actionable diagnostics (Python error message). The architectural principle: when a check fails, the message should tell the user *what to do*, not just *what's wrong*.

  **Scope:**
  - `server.py` — restore endpoint auto-install branch (~50 lines); PUT /api/config post-validation ntfy check (~30 lines)
  - `templates/config.html` — `loadNtfyStatus()` updated to call new `_updateNotificationsEnabledGate()` helper; live `input` event listener on the URL field; gate enforces the disable/re-enable based on URL + ntfy state (~70 lines of new JS, no removal)
  - `scripts/bootstrap.sh` — Python version check rewritten to distinguish broken-interpreter from old-version, family-aware reinstall hint
  - `uninstall.sh` — three `[N/5]` labels corrected to `[N/6]`
  - **No data-plane changes, no config schema changes, no sudoers changes** — purely UX hardening on the migration + setup paths.

  **Operational note:** the v3.3.0 prod migration hit the exact restore-stashes-ntfy pattern this fix addresses. The forward path on existing prod installs is unchanged — click "Install local ntfy" on the Notifications tab once, then the test notification button to verify the topic + base_url. After v3.3.1 deploys, future restores will auto-install ntfy if it ever needs to be re-installed.

## [3.3.0] — 2026-05-12

### Changed
- **Install location standardized to `/opt/aerodrome` on every distro, plus firewalld awareness.** v3.2.0's multi-distro work surfaced a real architectural question through dogfooding: on openSUSE Tumbleweed (and Fedora, RHEL, Rocky, Alma, AlmaLinux — anything SELinux-enforcing) installing under `/home/` doesn't actually work, because SELinux's targeted policy denies systemd from `ExecStart`ing a binary in a user's home directory. The user-visible symptom is a 203/EXEC failure on service start with "Permission denied" on `/home/<user>/aerodrome/venv/bin/python3` — even though Unix permissions allow it. v3.3.0 fixes this by moving the default install location to `/opt/aerodrome` on every distro, not just SELinux-enforcing ones. One install path, one mental model, FHS-correct, works on every distro without per-distro workaround documentation.

  **What you see as a new user:** `bash <(curl -fsSL https://install.aerodromeadsb.com)` now extracts to `/opt/aerodrome`, the bootstrap announces "Creating /opt/aerodrome (requires sudo since it's outside your home)..." once, you enter your password, and the rest of the install runs exactly as before. The systemd unit still runs as your regular user (not root), the config file is still user-owned and editable through the web UI, the in-app updater still works without sudo for file operations — `/opt/aerodrome` is created by root but then `chown`ed to the install user so everything downstream is unprivileged. The dashboard, watchlist, notifications, demo mode, switch-to-real wizard — all unchanged from a user-experience standpoint. The only difference is where the files live.

  **What you see as an existing `~/aerodrome` user:** absolutely nothing. v3.3.0 detects existing installs at any prefix (presence of `VERSION` + `main.py`) and upgrades them in place. No forced migration, no nag, no "your install is at the deprecated location" warning. If you want to move to `/opt/aerodrome` eventually, the procedure is documented in INSTALL.md (uninstall keeping data, reinstall fresh to `/opt`, restore your config + db via the web UI's Backup & Restore page). If you don't want to move, you never have to — `~/aerodrome` keeps working indefinitely.

  **Override:** `--prefix <path>` still works for users who have a strong preference. `bash <(curl ...) --prefix ~/aerodrome` restores the pre-v3.3.0 layout if that's what you want. The bootstrap warns nothing — it'll just install where you tell it.

  **Why /opt and not /usr/local or /srv or /var/lib:** `/opt/` is the FHS-blessed location for "add-on application software packages" — exactly Aerodrome's niche. `/usr/local/` is for system administrator-installed software intended to be managed by the package manager (which Aerodrome isn't). `/srv/` is for "data for services," not the services themselves. `/var/lib/` is also for data, not the application tree. `/opt/` matches conventional third-party software placement: GitLab, Splunk, Atlassian products, Sonatype Nexus, and dozens of others all install to `/opt/<product>/`. Aerodrome joins them.

  **firewalld awareness — the other Fedora/openSUSE finding.** Both Fedora and openSUSE Tumbleweed enable `firewalld` by default with a restrictive `public` zone that blocks port 8000. v3.2.x would install cleanly on those distros but leave the dashboard unreachable from any non-localhost source — which looked like "Aerodrome isn't running" from the user's laptop. v3.3.0's bootstrap detects active firewalld and prompts (default-yes) to open port 8000 persistently on the public zone. Skips silently on Debian/Ubuntu/Arch where firewalld isn't enabled by default. The exact commands are printed either way so users who decline can run them later.

  **`uninstall.sh` mirrors both changes:** it now detects whether the install is inside the user's home or outside (`/opt`-style) and emits the correct removal command — `rm -rf /opt/aerodrome` requires sudo because `/opt/` is root-owned, even though Aerodrome's own directory is user-owned. The "show command to remove the project directory" prompt and the `--purge` path both produce the right command for the install's actual location. Firewalld port 8000 also gets a "close it?" prompt during uninstall on firewalld-active systems, keeping install/uninstall state symmetric.

  **Why ship this as a minor (3.3.0) rather than a patch (3.2.2):** the install-location default change is user-visible behavior, not just bug fix. New users see a different default path. Existing users see no change but the install scripts know how to handle both layouts, which is new capability. Per semver, capability addition + default change is a minor bump. The release adds capability cleanly; it doesn't break anything.

  **Scope:**
  - `scripts/bootstrap.sh` — `PREFIX` default changed from `${HOME}/aerodrome` to `/opt/aerodrome`, install-dir setup step now creates parent dirs with sudo when needed and chowns the result, new "Existing install detected at <prefix> — upgrading in place" message when the target already has VERSION+main.py, new firewalld step between install.sh completion and the success banner. Help text and top-of-file comment block updated to reflect the new default.
  - `uninstall.sh` — install-dir-removal logic now branches on inside-vs-outside-home to emit the right command. Firewalld port-close step mirrors the install-side open step.
  - `install.sh` — no changes. install.sh has always used `$(dirname "$0")` to find the install root, so it adapts to wherever it's run from. The bootstrap is the only thing that decides where to extract.
  - `docs/INSTALL.md` — every `~/aerodrome` reference replaced with `/opt/aerodrome` (15 occurrences across the Installation, Updating, Backup, and Manual-install sections). New "Install location (v3.3.0)" and "Firewall handling" subsections in the Multi-distro support area documenting the new default, the override path, the upgrade-existing-installs behavior, and the firewalld prompt.
  - **Test contract**: maintainer dogfoods fresh openSUSE Tumbleweed and Fedora VMs end-to-end (install → demo mode → switch-to-real → uninstall) before publishing the GitHub Release. Arch and Debian/Ubuntu installs at their existing locations are also verified to upgrade cleanly without surprises.

## [3.2.1] — 2026-05-12

### Fixed
- **Fixed the empty server IP in the install epilogue URL on Arch and similar distros.** v3.2.0 added multi-distro support but left a Debian-specific assumption in place: both `install.sh` and `bootstrap.sh` use `hostname -I` (capital I) to grab the server's primary IP for the "Web UI: http://<ip>:8000" welcome line. That flag is a Debian extension to the `hostname` command — it doesn't exist on Arch's `inetutils` hostname, which is the standard hostname binary on Arch and Arch derivatives. The result: on Arch the command failed silently, `awk` got empty input, `SERVER_IP` was empty string, and the welcome URL rendered as `http://:8000` with no IP. Functional but confusing. (The service was actually running fine — just the post-install welcome banner missed the IP.)

  v3.2.1 replaces the single `hostname -I` call with a layered fallback chain that works across every modern Linux distro. The primary lookup uses `ip route get 1.1.1.1`, which is the universal approach: iproute2 ships with every systemd-based Linux distro (it's part of the base install on Debian, Fedora, Arch, openSUSE) and the `ip route get` subcommand consults the routing table without actually sending any network traffic, returning the source IP the kernel would use for a route to a given destination. That's exactly the "what IP should the user type into their browser" answer. Fallbacks: `hostname -I` (kept second so Debian behavior stays identical), then `hostname -i` (POSIX-ish; usable on stranger systems), then `"localhost"` as a final safety net so the URL is never blank. In practice on tier-1 distros the primary path always succeeds; the fallbacks exist as belt-and-suspenders for minimal container environments where iproute2 might be stripped.

  **Affected scope:** install.sh's "[5/5] Install complete!" epilogue and bootstrap.sh's "Bootstrap complete!" epilogue. Both surfaced the welcome URL with the same broken pattern, and both got the same four-line fallback chain. No other code uses the SERVER_IP value, so there's no other surface to update.

  **Not affected:** the runtime banner that `main.py` prints when the service starts (shows `http://{host}:{port}` where host is the bind address from `config['web']['host']`, typically `0.0.0.0` — that's a pre-existing display choice unrelated to this fix). The actual web service binding is unchanged. Existing Debian-family installs see byte-identical behavior because the primary `ip route get` lookup succeeds first and produces the same IP that `hostname -I` would have produced on those systems.

  **Why this slipped past v3.2.0's pre-release Arch dogfood:** the install completed successfully and the service started — the welcome banner's broken URL is cosmetic, not functional. Catching display issues requires reading the install output character-by-character, which is easy to skim past especially in a console terminal session. The fix is small and the root cause is clear, so it gets its own patch rather than rolling into a hypothetical v3.2.2.

## [3.2.0] — 2026-05-12

### Added
- **Multi-distro support: Fedora/RHEL, Arch, and openSUSE are now tier-1 supported alongside Debian/Ubuntu.** Until v3.2.0, the install bootstrap and `install.sh` hardcoded `apt-get` and Debian-style package names (`python3-venv` etc.) — they'd hard-refuse on a fresh Fedora or Arch host even though every line of Aerodrome's actual runtime code (collector, server, notifier, classifier, SQLite layer, FastAPI service) is fully distro-agnostic and just runs as a systemd-managed Python process. The Debian assumption was concentrated in the install/uninstall surface only. v3.2.0 fixes that: the bootstrap now detects the host's package-manager family from `/etc/os-release`, picks the right install command, and translates per-family package names automatically. The four tier-1 families covered with explicit detection rules and verified install paths are Debian (incl. Ubuntu, Raspberry Pi OS, Mint, Pop!_OS, elementary, Neon, Kali, Parrot), Fedora (incl. RHEL, Rocky, AlmaLinux, Amazon Linux, Oracle Linux), Arch (incl. Manjaro, EndeavourOS, Garuda, Artix, CachyOS), and openSUSE (incl. Tumbleweed, Leap, SLES). Each tier-1 family was dogfooded on a fresh VM before this release shipped.

  **What you see as a non-Debian user:** the same one-line curl install you'd use on Ubuntu now works on Fedora, Arch, or openSUSE without any flags. The bootstrap announces "Recognized Fedora/RHEL-family system" (or whichever) and proceeds. Step 1 of `install.sh` reports "Python3, pip, venv installed (via fedora package manager)" instead of just "installed" so you can see which family was detected. Everything downstream — virtualenv setup, systemd unit creation, sudoers rule, demo-mode synthetic feeder, switch-to-real wizard, in-app updater — works identically across families because all of it is systemd-and-filesystem operations that have always been distro-agnostic.

  **What you see as an existing Debian-family user:** absolutely nothing. The Debian path is byte-equivalent to v3.1.2 — same `apt-get update`, same `apt-get install python3 python3-pip python3-venv curl`, same systemd unit, same sudoers rule. The new code branches only activate when the detected family is non-Debian. No behavior change, no surprises, no upgrade gotchas.

  **The detection logic:** uses `/etc/os-release`'s `ID` field first (specific) and falls back to `ID_LIKE` (derivative) if `ID` doesn't match a known value — so most downstream derivatives that set `ID_LIKE` correctly (e.g. Pop!_OS reports `ID_LIKE="ubuntu debian"`) fall back to their parent family automatically without needing to be enumerated. The case statement covers ~20 distros explicitly across the four families. Anything else with systemctl AND one of apt-get/dnf/pacman/zypper on PATH falls to tier-2 (best-effort: prints a warning, prompts to continue, then uses whichever package manager it found). Tier-3 (hard refuse) only fires if systemd is missing OR no recognized package manager is present — covering Alpine, Void, Gentoo-without-systemd, and similar.

  **Per-family details (for the curious):**
  - **Debian/Ubuntu** — `apt-get update -qq` then `apt-get install -y -qq`. Packages: `python3 python3-pip python3-venv curl unzip`. venv is a separate package on Debian-family due to a longstanding split.
  - **Fedora/RHEL** — `dnf install -y -q`. No explicit refresh step (dnf auto-refreshes on install in normal usage; older yum needed it but RHEL 7's yum is past EOL). Packages: `python3 python3-pip curl unzip`. The venv module is bundled into the main `python3` RPM, so `python3-venv` doesn't exist and isn't installed.
  - **Arch** — `pacman -Sy --noconfirm` (refreshes the package index) then `pacman -S --needed --noconfirm` (install if not already present). The package is `python` not `python3` on Arch — Arch standardized on Python 3 long enough ago that the `python3` alias was dropped. Pip is `python-pip`. Packages: `python python-pip curl unzip`.
  - **openSUSE** — `zypper --non-interactive refresh` then `zypper --non-interactive install`. Packages: `python3 python3-pip curl unzip`. Modern Tumbleweed and Leap 15.5+ bundle venv into python3.

  **The shared abstraction.** The new `pkg_detect()`, `pkg_install()`, and `pkg_refresh()` functions are *duplicated verbatim* into `bootstrap.sh`, `install.sh`, and `uninstall.sh` rather than sourced from a shared library. This is deliberate, not accidental: the curl-installed `bootstrap.sh` runs from `/tmp` before any release zip is downloaded, so there's no shared library file it could source. ~70 lines of identical code in three files is honest cost; trying to be clever about it would mean either inline-from-zip (broken when there's no zip yet) or a separate downloaded helper (extra HTTP round-trip, more failure modes). The functions are also small and stable — package-manager command-line surfaces haven't changed in a decade, so the cost of duplication-drift over time is low. The shared logic includes the `/etc/os-release` parse, the family-detection case statement, the per-family install-command and package-name table, and the install/refresh wrappers.

  **Uninstaller now family-aware too.** Pre-v3.2.0, `uninstall.sh` ended with a hint reading "Remove manually if desired: `sudo apt remove python3-venv`" — useful for Debian users, confusing for everyone else. v3.2.0 detects the family the same way and emits the right removal command per distro (`sudo dnf remove ...` on Fedora, `sudo pacman -R ...` on Arch, `sudo zypper remove ...` on openSUSE). The actual destructive work of the uninstaller — stopping and removing both systemd units, removing the sudoers rule, removing the install directory — was already distro-agnostic; only the trailing informational hint needed updating.

  **Behind the scenes — what's left as Debian-specific.** Almost nothing. The Aerodrome runtime has no `apt`, `dnf`, `pacman`, or `zypper` references anywhere; the only code that knew about apt was the three install scripts. The optional ntfy-installer feature (`ntfy_installer.py`) was already distro-agnostic by design — it downloads the ntfy binary from GitHub Releases as a Go static binary rather than using any package manager. The systemd integration uses standard `systemctl` calls that work identically across all four families. The sudoers rule format (`/etc/sudoers.d/aerodrome`) is identical too. Python's venv, pip, and the FastAPI/uvicorn stack all work the same way regardless of which distro installed them.

  **What this enables next.** With multi-distro support landed, the option of containerizing Aerodrome (Docker image, OCI compatible) becomes much more interesting as a separate-but-complementary deployment option — a container is essentially "one more 'distro' to support," and the architectural separation we now have (data plane distro-agnostic, install surface family-aware) is exactly what makes a container build straightforward. That's on the longer-term roadmap rather than scoped for v3.2.0; the multi-distro work here is the right foundation.

## [3.1.2] — 2026-05-12

### Fixed
- **Fixed a NameError that crashed every install of v3.1.0 and v3.1.1 at startup, plus two defensive checks so this class of bug can't ship again.** v3.1.0 introduced the `/api/setup/switch-to-real` endpoint for the demo-mode switch-to-real wizard, with a type annotation reading `async def switch_to_real_execute(request: Request):` — but `Request` was never imported from `fastapi`. The release was AST-parse-clean (syntax was valid) but every install crashed at startup with `NameError: name 'Request' is not defined` the moment FastAPI evaluated the annotation. v3.1.1 carried the same bug forward because the fix only touched templates/updates.html and a single line of server.py response shaping — the bad annotation was untouched. v3.1.2 adds the proper `from fastapi import Request` to the top-level import line in server.py, allowing the endpoint to keep its standard FastAPI parameter-type annotation. No behavior change for users — the endpoint already worked correctly when it could be reached; the bug just prevented the service from starting up at all.

  **Operational note for users still on v3.0.x trying to upgrade:** v3.1.2 is the first release in the v3.1.x line that starts cleanly. If you haven't successfully upgraded past v3.0.x yet, deploy v3.1.2 directly (skip v3.1.0 and v3.1.1 entirely — there's no value in installing a release that crashes on startup). The recommended path remains the local-zip update flow: download v3.1.2 from GitHub Releases, drag it onto the `/updates` page's upload zone (or use `update-from-zip.sh` if you have it), then SSH to the server and run `./install.sh` from the install root once to refresh the sudoers rule to v4. After v3.1.2, every subsequent release will apply cleanly through the in-app GitHub channel with the fixed UX from v3.1.1.

  **Defensive check #1 — install.sh refuses to run from inside an existing install's `update/` subdirectory.** Today's separate prod-deploy mishap was triggered by exactly this: install.sh was run from inside `~/aerodrome/update/` (where the v3.1.1 zip had just been unpacked), `INSTALL_DIR` resolved to the staging directory rather than the install root, and the systemd unit was written pointing at `/home/<user>/aerodrome/update/venv/bin/python3` — which doesn't exist, because the venv lives in the install root, not staging. The service then exited 203/EXEC on every restart attempt. The new guard checks at install.sh startup: if the script's directory is named `update` and its parent has a `VERSION` file plus a `main.py`, it's sitting inside an existing install's staging area; refuse to run with a clear error pointing at the real install root (`cd <parent> && ./install.sh`). The check is silent in the normal install path — only fires when the footgun is actually being approached.

  **Defensive check #2 — static name-resolution check on every release bump, REQUIRED (not advisory).** Added to `bump-version.sh`. Walks every function annotation in `server.py`, collects all module-scope names (imports, top-level definitions, nested class names that might be Pydantic models), and flags any uppercase name referenced in an annotation that isn't in the set of defined names. Today's `Request`-without-import bug fails this check loudly with `server.py:8816  'Request'` and exits non-zero, blocking the bump. The check is intentionally REQUIRED rather than advisory (unlike `scripts/check_docs.py`) because a release that crashes every install at startup is broken, not "drifted." A `--skip-name-check` escape hatch exists for documented false positives — Pydantic BaseModel subclasses defined inside `get_app()` for endpoint payloads, for example — but it requires explicit opt-in, paired with a comment about *why* the skip is safe. The check correctly passes on the v3.1.2 codebase and was verified by re-introducing the exact bug from v3.1.0/3.1.1 in a test pass, where it fired with the expected output.

  **Why AST parse alone wasn't enough:** Python's `ast` module checks that source text *can be parsed*, not that it *resolves at runtime*. `def f(x: Foo)` parses fine even when `Foo` is undefined — the annotation is just an AST node referencing a Name. The error only fires when Python tries to evaluate the annotation, which happens at function-definition time (i.e. module import). For a fast-feedback dev loop where the server actually starts after every change, this catches itself — but for a release pipeline where the bump packages the zip without doing an end-to-end import test, the bug can slip through unless something specifically checks name resolution. v3.1.2's `bump-version.sh` check is that something.

  **Scope of changes:**
  - `server.py` — one-character addition to the `from fastapi import ...` line on line 30 (`, Request` appended). Endpoint signature on line 8816 keeps its standard `request: Request` annotation.
  - `install.sh` — new guard block at the top, immediately after `INSTALL_DIR` is computed. Refuses to run from inside `update/` of an existing install with a clear error and pointer.
  - `bump-version.sh` — new static name-resolution check between the argument-parse step and the documentation-drift check. Runs by default on every bump; can be skipped with `--skip-name-check` for documented false positives. New `--skip-name-check` flag added to the arg parser and the usage docstring.

  **No data-plane changes, no template changes (beyond the version-header on `templates/switch-to-real.html` which gets a stale-version drift flag automatically), no config-schema changes, no sudoers changes** (SUDOERS_VERSION stays at 4). v3.1.2 is purely a fix release for the startup-crash bug + two release-pipeline guardrails to make sure the same class of bug can't ship again.
...
### Added
- **Demo mode for users without an ADS-B receiver.** Until now, the answer to "I'd like to try Aerodrome but don't have a receiver yet" has been: don't, because the dashboard would just be empty. v3.1.0 changes that. Pass `--demo` to the bootstrap (or pick option 2 at the interactive "Do you have a receiver?" prompt) and Aerodrome installs in demo mode with a small synthetic feeder running alongside it. The dashboard fills with 50 simulated aircraft, a starter watchlist of 8 deterministic "regulars," ~5% in the US military hex range so the military classifier exercises, and the occasional emergency squawk (max one per hour) so the alert path is visible. When you have a real receiver ready, the in-app switch-to-real wizard at Configuration → Demo handles the destructive transition cleanly — test reachability of the new receiver first, stop and remove the feeder service, nuke the demo database, clear the demo watchlist, and restart aerodrome pointed at your real address. The whole arc from "I want to try this" to "I've connected my real receiver" lives entirely inside the install + dashboard, no terminal required beyond the one-line install.

  **What you see in demo mode:** A persistent yellow banner across every page reading *"Demo mode — showing simulated data. Configure real receiver"* with a link to the switch-to-real wizard, so there's no risk of forgetting the dashboard is simulated. Every push notification (if you've configured ntfy) arrives prefixed with `[DEMO]` so messages announce themselves as synthetic rather than alarming you with what looks like a real alert — the prefix disappears automatically when the wizard flips you to real mode, no notifier reload required. Clicking any "Track ↗" link on an aircraft pops a confirmation dialog explaining that the ICAO is simulated and won't be found by external trackers; "Continue anyway" still opens the link (respects curiosity), "Cancel" closes the dialog. An 8-aircraft starter watchlist seeded at install time, labelled "Demo: regular #1" through "#8", surfaces watchlist hits as the deterministic regulars come into range.

  **Behind the scenes — the architecture choice:** the synthetic feeder runs as a *separate systemd service* (`aerodrome-synthetic-feeder.service`) bound to `127.0.0.1:8080`, not as an in-process module. The main aerodrome.service polls the same URL it would poll a real ADS-B receiver — it has no idea the data is synthetic. This was a deliberate split: the collector, classifier, notifier, and database pipeline stay completely ignorant of demo mode, no demo-specific branches in the data plane. Only the UX layer (banner, notification prefix, Track-link guard) reads the `demo.enabled` config flag. The fleet seed is locked to `1903` (Wright Brothers, first powered flight) so every demo install everywhere sees the exact same simulated aircraft, and the seed_watchlist tool that picks the starter watchlist's 8 ICAOs uses the same seed so the watchlisted aircraft are the actual "regulars" the feeder generates. Service-restart safety: if the feeder isn't up yet when aerodrome.service starts polling, the collector's existing receiver-unreachable retry behaviour handles it gracefully — no special demo-aware ordering needed.

  **The switch-to-real wizard:** a 4-step flow at `/setup/switch-to-real`. Step 1 is the confirm-intent screen with an explicit warning that switching deletes the demo database, demo watchlist, and accumulated stats (per design, this is destructive — carrying simulated sightings into real-receiver mode would pollute all-time stats forever). Step 2 prompts for the real receiver's IP, port, optional latitude/longitude, and optional path override. Step 3 executes with live per-step progress: the server tests reachability of the new receiver first (catches typo'd IPs *before* destroying anything), then stops + disables + removes the synthetic-feeder unit file, deletes `aircraft_history.db` and its WAL/SHM siblings, rewrites `config.yaml` to flip `demo.enabled=false` and set the real receiver details, and restarts aerodrome.service. Step 4 is the confirmation page pointing back at the dashboard. If any step fails, the wizard displays which one and offers a back button to fix the inputs — the demo install is unchanged on early-step failures (reachability test is intentionally first for this reason).

  **Three install paths to demo mode**, all functionally equivalent:
  1. *Curl one-liner, interactive*: `bash <(curl -fsSL https://install.aerodromeadsb.com)` → pick option 2 at the prompt.
  2. *Curl one-liner, scripted*: `bash <(curl -fsSL https://install.aerodromeadsb.com) --demo --lat 44.84 --lon -88.23`.
  3. *Manual zip install*: download the release zip, unzip, `./install.sh --demo --home-lat 44.84 --home-lon -88.23`. This third path is fully standalone — `install.sh --demo` patches `config.yaml` to flip `demo.enabled=true` and point `receiver.ip/port` at the synthetic feeder, installs the feeder systemd unit, runs the watchlist seeder, and extends the scoped sudoers rule so the switch-to-real wizard's `systemctl stop/disable` calls work without a password. Manual zip installs of the real-receiver flow (no `--demo` flag) are completely unchanged from v3.0.x.

  **Uninstall awareness:** `./uninstall.sh` is now aware of the synthetic-feeder service. On any install where the feeder unit exists, uninstall stops + disables + removes it alongside the main aerodrome.service — no special demo-mode uninstall flag needed, and non-demo installs see "no feeder service found" rather than error noise.

  **What v3.1.0 touches:**
  - **New top-level config key:** `demo.enabled` (bool, default false). Validated in `config_validator.py`. Section commentary in `config.yaml.example` explicitly warns users not to hand-edit — the switch-to-real wizard is the supported transition.
  - **New systemd unit shape:** `aerodrome-synthetic-feeder.service`, installed by `install.sh --demo`, removed by the switch-to-real wizard or by `uninstall.sh`.
  - **Notifier `[DEMO]` prefix:** ~10 lines in `notifier.py` — applied at the top of `notify()` (catches every event including daily_summary which routes through notify) and at the `send_test()` call site (which bypasses notify for event-gate reasons). The flag plumbs through `_build_notifier()` / `_refresh_notifier_config()` so live config edits flip the prefix without a notifier rebuild.
  - **`/api/status.demo_enabled` field:** so admin pages can read demo state without their own status fetches. Every admin template already polls `/api/status` for the gear-menu health badge.
  - **Centrally-injected demo banner:** in `static/theme.js`. Self-installs across all 11 admin templates (theme.js is shared); injects the banner element as `document.body`'s first child so it sits above the header on every page regardless of layout. No per-template edits needed. Also exposes `window.aerodromeDemoMode` for the Track-link guard.
  - **External-link guard:** delegated click handler in `static/theme.js` targeting `a.track-external-link`. Both `templates/index.html`'s `trackLink()` helper and `templates/aircraft.html`'s `#trackLink` button now emit this class, with a `data-icao` attribute so the dialog can show the user which aircraft they're trying to track.
  - **`/config` Demo tab:** new section (id `demo`, between Updates and Backup & Restore in the tab list). On demo installs, shows a status summary ("what's running") and a prominent "Switch to real receiver" launcher. On real installs, shows a friendly explainer of what demo mode is and notes that it can only be entered via a fresh install (no in-app "switch to demo" — that would mean discarding real receiver data, which has no legitimate use case).
  - **`/setup/switch-to-real` route and template:** new dedicated URL for the wizard. Lives at its own URL (rather than embedded in /config) so the browser back button does the right thing during the multi-step destructive flow. Renders `templates/switch-to-real.html` which is a 4-step single-page wizard with JS step-state-machine.
  - **`/api/setup/switch-to-real` POST endpoint:** server-side handler for the transition. Returns `{ok, error, steps}` so the wizard can advance or display the failure point. Defensive guards: refuses to run on non-demo installs (defends against accidental nukes via stale browser state), validates required inputs, reachability-tests the new receiver before destruction.
  - **Synthetic feeder additions** (`tools/synthetic_feeder/generator.py`): emergency-squawk event state machine on `Fleet`, with a strict 1/hour cooldown enforced by `_next_emergency_eligible_at`. The `emergency` field on each aircraft snapshot is now computed from `squawk` rather than hardcoded `"none"` — so when an emergency event marks an aircraft with squawk 7500/7600/7700, the emergency field correctly reads `"hijack"`/`"radio"`/`"emergency"` exactly as readsb would emit.
  - **Watchlist seeder** (`tools/synthetic_feeder/seed_watchlist.py`): new one-shot script. Picks the first 8 ICAOs the deterministic fleet would generate at install time, then pattern-substitutes them into `config.yaml`'s `watchlist: []` line preserving all surrounding comments (same approach `bootstrap.sh`'s config patcher uses). Idempotent — bails if the watchlist is already non-empty.
  - **SUDOERS_VERSION bumped 3 → 4:** the new feeder-service lifecycle rules require a sudoers refresh on upgrade from v3.0.x to demo-capable v3.1.0. The updater's existing drift-detection surfaces this in the UI; users get the same "Sudoers update required" modal that v3.0.x deploys use, no new install pattern.
  - **Documentation pass:** README's Install section gains a demo-mode paragraph; `docs/INSTALL.md` gains a full Demo mode section between Installation and Remote access (with full subsections on the three install paths, what's running, the user-visible surfaces, and the switch-to-real flow); `docs/overview.md` gets a brief mention in Getting Started; `ARCHITECTURE.md` gets a Demo mode architectural note explaining the separate-process choice and the single-flag/three-surfaces structure; CONTRIBUTING.md gets a forker-pointer bullet pointing at `tools/synthetic_feeder/` and the seed-lock convention. The README Project Structure listing now includes `tools/synthetic_feeder/` with all four bundled files.
  - **PDF + screenshots regenerated.** v3.1.0 runs `bump-version.sh` without `--skip-docs-drift` per the working convention; drift check passes clean.
...
### Added
- **Announced the curl-install URL in the README and the in-app gear menu.** Completes the bootstrap arc that ran from v3.0.12 (promotion out of draft) through v3.0.13 and v3.0.14 (two bugs surfaced and fixed by clean-VM dogfooding). v3.0.14 ran end-to-end cleanly on a fresh Ubuntu 24.04 VM — every step from "Checking platform" through "Tracker is running!" completed without intervention, the colorized URL rendered correctly, the auto-detected timezone went into config.yaml silently, and the web UI was reachable at the printed URL. With the install flow verified working, the URL `install.aerodromeadsb.com` is now promoted from "live but unannounced" to "the canonical install method."

  **Two announce surfaces in v3.0.15:**

  **The README Install section now leads with the one-liner.** The section opens with "One command:" followed by `bash <(curl -fsSL https://install.aerodromeadsb.com)` and a paragraph explaining what the bootstrap does (platform detection, prereq install, GitHub Releases download, SHA256 verification, prompt for receiver-side config, install.sh handoff). The manual install path moved into a collapsed `<details>` block titled "Manual install (alternative)" — it stays available for users who want to inspect the bootstrap before running it, install offline, or pin to a specific release version, but it's no longer the first thing a prospective user reads. The collapsed block also documents the bootstrap's `--from-zip` flag for offline installs and references `--help` for the full flag list. The Project Structure listing's `scripts/bootstrap.sh` entry dropped its now-stale "(draft)" qualifier and re-described as "curl-installable one-line setup (see Install)" so the cross-reference points users at the install section where the one-liner actually lives.

  **The in-app gear menu now has an install-share surface.** Below the existing Theme picker (and above nothing — it's the new bottom of the menu) lives a small block with the label "Install Aerodrome elsewhere", the install command in a monospace code element, a "Copy" button next to it, and a link to the GitHub repo. The use case is the one the maintainer flagged: "where someone showing Aerodrome to a friend can point them at it." Click the gear, see the install URL, click Copy, paste in a message to your friend, done. The copy handler tries the modern `navigator.clipboard.writeText` API first and falls back to the older `textarea + execCommand('copy')` pattern for non-secure contexts (Aerodrome is typically accessed over plain HTTP on a LAN, so the modern clipboard API isn't always available — the fallback gets exercised more often than not in practice). Visual feedback: the button text flips to "Copied!" for 1.4 seconds. The block uses the same uppercase-label + contained-widget visual language as the Theme picker above it, so the menu's bottom section reads as a coherent "tools + info" group rather than two unrelated pieces. CSS landed in `static/theme.css` next to the existing theme-row rules so future maintainers find both gear-menu-bottom blocks colocated.

  **Why the gear menu rather than a dedicated About page.** Considered three options: (1) add an "About" gear menu item that opens a new `/about` page with version + install URL + repo link + license + third-party credits, (2) add a modal triggered from a gear menu item, (3) embed directly in the gear menu. Option 1 is the most distinctive and would also be a natural home for license info and third-party credits (Leaflet, OSM, CARTO — currently only in the README), but it requires a new route in `server.py`, a new `templates/about.html`, and a sixth gear-menu link competing with the existing five action items. Option 2 needs modal infrastructure that isn't quite there in `index.html` (the `alertModal` reference at line 5504 is a forward-reference to status.html's modal, not a usable one here). Option 3 — embedding directly in the gear menu — is the lowest-overhead path and fits the "showing it to a friend" use case perfectly (the friend is presumably already looking at the dashboard, so they can see the URL the moment the gear opens; no extra clicks). The future About page is queued — see queue item #29.

  **Net change:** ~60 lines across three files. README install section rewrite (~45 lines); gear-menu HTML block in `templates/index.html` (~13 lines); copy-handler JS in same file (~40 lines including the navigator.clipboard + fallback pattern); CSS in `static/theme.css` (~35 lines for the four `.install-share-*` rules). No new routes, no schema changes, no behavior change to the running service or the channel — pure announce surface on top of the v3.0.14 verified-working bootstrap. The arc that started with `install.aerodromeadsb.com` going live as a DNS-only surface earlier today closes here: live + verified end-to-end + announced in both the public README and the in-app surface.

  **What's NOT in v3.0.15** (filed forward): a dedicated About page (queue item #29 — could grow into the natural home for license info, third-party credits, links to GitHub/docs/changelog, and the install command); the first-time setup wizard (queue item #28 — would pair naturally with the install-share surface by giving fresh users a guided first-run flow); and the broader Tier-2 polish queue. The bootstrap is done; the rest is the next day's work.

## [3.0.14] — 2026-05-11

### Fixed
- **Polished two `scripts/bootstrap.sh` papercuts surfaced by the v3.0.13 clean-VM dogfood.** Both made it through end-to-end the second time around (the install succeeded and the service started cleanly), but they degraded the first-impression on a fresh user enough to be worth fixing before announcing the install URL.

  **Papercut 1: color-escape leak in the final banner.** The very last line of the bootstrap's "Bootstrap complete!" output uses string interpolation to colorize the web-UI URL: `"Aerodrome is running at: ${CYAN}http://${SERVER_IP}:8000${RESET}"`. Every other color-using line in the script uses `echo -e` so the `\033[...` escape sequences embedded in the color variables get interpreted as actual ANSI codes by the terminal; this one line used plain `echo` (no `-e`), so the escapes printed as literal text — `\033[36m\033[1mhttp://...:8000\033[0m` showing up in the terminal output instead of cyan-bold "http://...:8000". The fix is adding `-e` to that one echo. Grep confirmed it was the only outlier in the entire script. ~1-line edit.

  **Papercut 2: timezone prompt accepted invalid input.** The interactive timezone prompt asked for "Time zone (IANA)" with the system-detected timezone as the default, but accepted any string the user typed — including common-but-wrong inputs like "CDT" (a US timezone abbreviation, not a valid IANA name). On the v3.0.13 dogfood the user typed "CDT" at the prompt and the install completed without complaint; the invalid value got written to `config.yaml`, where it would have caused the running service to fail to localize timestamps the first time it tried (silently falling back to UTC for display, or crashing on `zoneinfo.ZoneInfo("CDT")` depending on which code path got there first). The fix shifts the philosophy entirely: instead of asking the user for the timezone and validating their answer, **the timezone is no longer prompted** — the auto-detection that was already running (line ~530, reading from `/etc/timezone` or `timedatectl`) is used silently, and the user is pointed at the web UI's Configuration page to adjust the timezone or any other setting after install. Reasoning: the detection sources only produce valid IANA names by construction (`/etc/timezone` and `timedatectl` both emit canonical names), the system tz is almost always what the user wants on their own machine, and removing the prompt eliminates the whole class of "user types something plausible that happens to be wrong" bugs. The `--timezone` flag path stays for scripted overrides where power users know what they're doing. A short `log_info` line during step 6 confirms the auto-detected value so the user can see what was picked: "Time zone: America/Chicago (auto-detected; change later in web UI → Configuration)".

  **Companion change to the post-install banner.** Since the bootstrap now picks several defaults silently rather than asking (timezone now, more likely later — see the queued first-time-setup-wizard item), the banner needs to point users at where they go to customize. Rewrote the final block from "Open the URL in a browser to finish setup. Watchlist, notifications, retention, and display preferences are all configurable from the web UI." to a more directive version that calls out the gear-menu → Configuration path explicitly and yellow-highlights "Next step:" so it visually pops as the actionable thing to do after the URL line. The new copy: "Next step: open the URL above, then visit gear menu → Configuration to review and adjust settings (timezone, watchlist, notifications, retention, display preferences, and more). The install picked sensible defaults but most users will want to customize at least a few of them."

  **What's NOT done yet:** the planned first-time-setup-wizard inside the web UI itself, with an in-app button to re-launch it if the user dismisses it. Filed for future work. The bootstrap-side change here narrows the install prompt count and makes the post-install handoff to the web UI more explicit, which sets up nicely for whatever wizard pattern lands later — at the limit, the bootstrap could be reduced to just receiver IP/port and everything else handled in the web UI wizard. Not in v3.0.14's scope.

  **Net change:** ~30 lines edited across `scripts/bootstrap.sh` — one-line echo fix for Papercut 1, ~15 lines replacing the timezone prompt block with silent auto-detection + log_info, ~10 lines rewriting the final banner copy to point at the Configuration page. Bootstrap header version bumped to `3.0.14`. No other files touched. No behavior change to the running Aerodrome service — every change is install-flow-only.
...
### Changed
- **Promoted `scripts/bootstrap.sh` out of draft state.** The bootstrap script that handles curl-installable Aerodrome installs has carried a "DRAFT" marker since the early scaffolding days — header version `3.0.3-draft`, title line ending in "(DRAFT)", and a multi-paragraph DRAFT NOTICE block at the top listing four conditions that needed to be true before the script would actually work: the GitHub repo had to be public, the Releases-based publishing flow had to be live, the GitHub Releases API endpoint had to return real release data, and `install.aerodromeadsb.com` had to redirect to the canonical bootstrap URL on `raw.githubusercontent.com`. All four are now satisfied: the repo went public earlier today, eleven Releases have shipped through the v3.0.x cluster, the API endpoint returns real data (the in-app update channel polls it every check cycle), and the Cloudflare DNS + Single Redirect that took `install.aerodromeadsb.com` live was verified end-to-end earlier this session. The DRAFT NOTICE block is therefore stale and actively misleading — a user reading the script would see "NON-OPERATIONAL until..." conditions that are demonstrably already met.

  Three changes land in v3.0.12: (1) header version bumped from `3.0.3-draft` to `3.0.12` to match the parent release (and to make `--help` output a real version string rather than a placeholder), (2) title line scrubbed from "Aerodrome — Curl Install Bootstrap (DRAFT)" to just "Aerodrome — Curl Install Bootstrap", (3) the entire DRAFT NOTICE block removed and the "Headline command (post-launch)" / "Local testing today" lines collapsed into a clean "Headline command" / "Local testing" pair (the post-launch qualifier no longer applies; the local-testing example was pinned to a v2.97.4 zip filename which is ancient — replaced with a generic `aerodrome-vX.Y.Z.zip` placeholder that doesn't go stale on every release). The `show_help()` heredoc at the bottom of the header was already clean (already used the generic `vX.Y.Z` placeholder) so no change needed there.

  No functional change. Same prereq detection, same OS-compat handling, same version resolution, same download/verify, same prompts, same install.sh handoff. The DRAFT scrub is purely a cosmetic and documentation correction reflecting that the script's prerequisites are now real.

  **What's NOT in v3.0.12 yet:** an end-to-end clean-VM dogfood of the actual install flow. The bootstrap has been exercised locally via `--from-zip` paths many times and the curl-redirect surface was verified through HTTP-level smoke tests (302 + correct Location header + Cloudflare server header + content match), but no fresh Ubuntu droplet has actually run `bash <(curl -fsSL https://install.aerodromeadsb.com)` end-to-end through the prereq install + GitHub fetch + zip download + sha verify + interactive prompts + install.sh handoff + service start. That dogfood is the gate on announcing the install URL in the README and the in-app About panel; until it passes, the URL stays live but unannounced (working for anyone who already knows it, not promoted to new users). Whichever release follows v3.0.12 (presumably v3.0.13) will land the README + About-panel announce after the dogfood succeeds — splitting promotion from announce keeps the README on `main` from referencing an install URL that hasn't been end-to-end verified.

## [3.0.11] — 2026-05-11

### Fixed
- **Improved the v3.0.10 cross-field validation message and made it clickable.** The v3.0.10 validator correctly refused `updates.github.notify.ntfy: true` when ntfy wasn't configured, but the error message had three rough edges that surfaced when the validation actually fired in real use: the phrasing was technical and a little cold ("Cannot enable update push without ntfy configured"), it named raw YAML key paths in user-facing copy ("Set notifications.enabled: true on the Notifications tab"), and it offered no way to navigate to the place the user needed to fix things. The new message reads "Configure ntfy on the [Notifications tab](/config#notifications): turn ntfy on, set a URL." — shorter, imperative, matches the voice of every other validator message in the codebase, and the "Notifications tab" portion is a real clickable link that takes the user directly to where they need to be. The "turn ntfy on, set a URL" suffix adapts based on which pieces are missing: both → "turn ntfy on and set a URL", only the master switch → "turn ntfy on", only the URL → "set a URL". One click from error to fix.

  The link works because the `field-error` renderer injects the validation message via template literal without escaping (line 1021 of `templates/config.html`), so a real `<a>` tag in the message string renders as clickable HTML. This is safe in our specific case because validator messages come from server-side Python code hardcoded in the codebase — never from user input — so there's no XSS surface even though the renderer doesn't sanitize. The target URL is `/config#notifications` which uses the existing v2.42.1 URL-hash tab-routing convention.

  **Required a companion fix to complete v2.42.1's hash-routing feature.** v2.42.1 read the URL hash on page load to set the initial active tab (so `/config#notifications` would open the Notifications tab when navigated to from another page or from a bookmark), and updated the hash via `history.replaceState` whenever the user clicked a tab button. But it never registered a `hashchange` event listener, meaning if the hash changed *while the page was open* (clicking an `<a href="/config#x">` link, hitting browser back/forward, manually editing the URL), the tab wouldn't actually switch — only the URL would update. That gap was invisible until v3.0.11's improved validation message introduced an in-page link that depends on hash-driven tab switching. The fix is a 5-line addition right after `switchTab()`: `window.addEventListener('hashchange', ...)` that calls `switchTab()` when the hash changes externally. Net effect beyond the validation use case: browser back/forward across tabs now works naturally (the back button on the Updates tab now returns to whatever tab the user was on previously, rather than navigating away from `/config` entirely), and any future in-page deep links to specific tabs will Just Work.

  **Architectural slot used:** the validator change is contained to the existing `validate_config()` cross-cutting-checks slot at the bottom of `config_validator.py`. The hash-listener change is a sibling to `switchTab()` in `templates/config.html`, placed immediately after the function it depends on with a comment explaining the v2.42.1 archaeology so the next maintainer doesn't have to reconstruct why the listener exists.

  ~25 lines of code change total (10 lines in the validator for the new message format and the adaptive copy logic, 8 lines in the template for the hashchange listener). No new dependencies, no schema changes, no behavior change to the channel or the notifier — pure validation-message and tab-navigation improvements on top of unchanged underlying infrastructure.
...
### Fixed
- **Scrubbed a write-time PII leak in the v3.0.8 CHANGELOG entry.** The v3.0.8 entry as originally written named the maintainer in describing how the validator regression was caught. That violated the standing distribution requirement that release-bound files (anything in the shipped zip, including CHANGELOG.md which feeds the public GitHub Release body) be free of maintainer-name references — only the `LICENSE` MIT copyright holder and the `preston-peterson` GitHub username embedded in repository URLs are approved exceptions per the v2.97.7 PII review. The fix in v3.0.9 rewrites the offending sentence into actor-agnostic framing — same technical content, same explanatory voice, no named maintainer. The v3.0.8 commit and Release body on GitHub still contain the original phrasing in `git log` history; the v3.0.9 commit replaces it going forward. Maintainer can optionally edit the v3.0.8 Release body on GitHub to redact the leak from the Release-page surface separately if desired — Release body text isn't tied to commit history.

  Root cause of the write-time miss: today's release cadence (nine releases in a single session) compressed the normal CHANGELOG-writing pass, and the actor-named phrasing slipped through. The standing rule is "PII discipline at write-time, not audit-time" — meaning the changelog should be written without maintainer names from the start, not scrubbed after the fact. The compressed cadence made me lean toward more conversational phrasing that names who-did-what; the fix going forward is to default to the actor-agnostic framing (use "the maintainer," "surfaced during," passive constructions) for every release-bound file regardless of how casually the session is moving.

  No code change in v3.0.9. Pure CHANGELOG correction.

## [3.0.8] — 2026-05-11

### Fixed
- **Fixed a v3.0.2-era validator regression that prevented users from enabling the ntfy push-notification feature v3.0.2 shipped.** Surfaced during v3.0.7 dogfooding: navigating to the new `/config` → Updates tab and toggling "Send ntfy push notification," then flipping over to the Notifications tab (per the v3.0.7 helper text on the Updates tab explicitly instructing this) to enable `notifications.events.update_available` — the second half of the two-key gate every ntfy event uses. Save returned `1 validation error(s)` with message `"Unknown event type (known: capacity_low, capacity_recovered, daily_summary, emergency_squawk, new_record, receiver_offline, receiver_recovered, special_aircraft, watchlist_hit)"`. Root cause: when v3.0.2 added `update_available` to the ntfy event registry, it correctly updated `notifier.py`'s `KNOWN_EVENTS` set (the runtime-side gate that decides whether the notifier accepts a `notify(event="update_available", ...)` call) but missed the parallel `known_events` set in `config_validator.py` at line 612 (the save-time gate that decides whether `notifications.events.update_available: true` is a valid YAML key). Two parallel sources of truth for the "what events exist" question, only one updated. The runtime would have fired the notification correctly if the validator had let the user save the config to enable it — but the validator gated the save, so the notification never had a chance to fire.

  The fix is a one-line addition to the `known_events` set in `config_validator.py`'s `_validate_notifications()` block, with a comment explaining the v3.0.8 archaeology so the next time someone adds a new event type, they remember to update both lists. Net diff: ~10 lines (the literal addition plus the explanatory comment).

  **The deeper lesson:** any new ntfy event type now needs updates in three places — `notifier.py` `KNOWN_EVENTS` (runtime), `config_validator.py` `known_events` (save-time validation), and `config.yaml.example` `notifications.events` block (default config). The first two should arguably be unified to a single source of truth, possibly by importing `KNOWN_EVENTS` from `notifier.py` into `config_validator.py` rather than maintaining a parallel set. Filed for future polish; not in v3.0.8's scope.

  **Verification path:** users on v3.0.7 who tried to enable update_available push notifications and hit "Unknown event type" can now save successfully after applying v3.0.8. The feature was *fully implemented* in v3.0.2 (notifier registry, transition detection, ntfy route mapping, cooldown — all working) and *fully reachable in the UI* in v3.0.7 (Updates tab + Notifications tab), but blocked at the validator from v3.0.2 through v3.0.7. v3.0.8 is when the feature actually becomes enable-able by an end user.

  **The other UX gap surfaced here, filed for future polish:** the `/config` status bar shows "N validation error(s)" without surfacing *which* error or *which* field. Per-field inline error display does exist, but only renders on the tab containing the error, and only if the user happens to be looking at that field. Auto-switch-to-error-tab logic exists too but only fires when the current tab has zero errors. Net effect: a user editing one tab who hits a validation error on a different field on a different tab can see "1 validation error(s)" without any indication of where to look. Two improvements worth queuing: (1) include the first error's path-and-message in the status bar text (e.g. `"1 validation error: notifications.events.update_available — Unknown event type"`), and (2) make the count clickable to scroll to / focus the first error field. Both are small (~30 lines each); both meaningfully improve the debugging experience. Filed for v3.0.9+.

## [3.0.7] — 2026-05-11

### Added
- **Added an Updates tab to the `/config` page exposing the GitHub-update-channel settings in the web UI.** Closes a gap that had been latent since v3.0.0: the `updates.github.*` config keys (master enable, poll cadence, three notification surfaces) were live in production via the channel infrastructure but had no web-UI presence — users had to edit `config.yaml` directly or use the raw-YAML editor to change cadence from the default `monthly`, toggle notification surfaces, or disable the channel entirely. v3.0.7 adds a dedicated Updates tab in `/config` between Notifications and Backup & Restore, with five fields covering all the channel's tunables: `updates.github.enabled` (master switch), `updates.github.poll_interval` (daily/weekly/monthly/never dropdown), `updates.github.notify.banner` (the in-card "vX.Y.Z is available" banner on `/updates`), `updates.github.notify.gear_badge` (the amber dot on the gear menu when an update is available), and `updates.github.notify.ntfy` (push notification, with helper text explicitly calling out the two-key gate with `notifications.events.update_available` on the Notifications tab — same pattern every ntfy event uses).

  **Why a new tab rather than folding into Notifications or replacing the gear menu's "Check for updates" item.** Considered three reorganizations and landed on "add the tab, keep the gear menu item." The other surfaces in the app already follow a consistent split: pages for status/action, config tabs for settings. Status has `/status` (observe) but no Status tab in `/config` because Status isn't configurable; Logs has `/logs` (read) and a Logging tab (where retention and log level live, which ARE configurable); Notifications has no real-time page but does have a Notifications tab (config-only). Updates was the inconsistent one — it had a `/updates` page (real-time view: what version am I on, what's available, click Apply) but no corresponding config tab for the *settings* about how often that check happens and which surfaces get notified. Adding the Updates tab regularizes the pattern rather than creating a new one: the page is for *acting* on updates ("is there an update right now?"), the tab is for *configuring* the channel ("how often should it check?"). Different verbs, different surfaces. The gear menu's "Check for updates" item stays as it is.

  **Tab placement and naming.** Inserted as the tenth of eleven tabs, between Notifications and Backup & Restore. Notifications neighbor makes sense because some of the updates-side settings (the ntfy push toggle) interact directly with notification configuration — users adjusting one will often want to adjust the other. Backup & Restore stays the last tab because it's an operational tool rather than a settings group, and it's structurally different from every other tab (it triggers actions rather than editing values). The tab is named "Updates" (singular short-form) to match the brevity of every other tab label rather than "Update channel" or "GitHub updates" — terser scans better in the tab strip and the section headers inside the tab provide the longer-form context.

  **Section structure within the tab.** Two sections rather than one. The first ("GitHub-based update channel — discovery, polling cadence") covers the master enable plus poll_interval, framed by an intro paragraph explaining how the discovery loop works (background thread on a schedule, page loads serve cached state, "Check now" forces an immediate check regardless of cadence). The second ("Notification surfaces — where you find out about new releases") covers the three notify toggles, framed by an intro paragraph distinguishing passive surfaces (banner, badge — visible only when you visit the dashboard) from the active surface (ntfy push — pings your phone). The split keeps each section visually scannable and groups the toggles by what they conceptually mean rather than by their position in the YAML.

  **Live-config callout.** A small dimmed panel at the bottom of the Notification surfaces section reminds users that changes here take effect within ~1 hour without a service restart, because the `updates` section is in `LIVE_KEYS` and the scheduler re-reads `CONFIG` on each wake. This is true for any of the five new fields — toggle the channel off, change cadence, flip a notify surface, anything — and worth saying explicitly so users don't think they need to restart after editing.

  **No new config keys, no schema migration, no behavior change to the channel itself.** All five fields bind to keys that have existed in `config.yaml.example` since v3.0.0 (the channel infrastructure release) plus the v3.0.2 ntfy addition. The validator already accepts all five keys correctly. The `LIVE_KEYS` membership is already in place. This release is purely UI on top of unchanged underlying infrastructure. ~130-line addition to `templates/config.html` (one TABS entry, one section render function with two `.section` blocks and five `field()` calls); no other files touched.
...
### Added
- **Closed the v3.0.x arc — ntfy push notifications for newly-discovered Aerodrome updates.** With v3.0.0 (check + discover) and v3.0.1 (in-app apply) live, the GitHub channel had two of three notification surfaces wired: the gear-menu amber dot across admin pages, and the in-card status on `/updates`. v3.0.2 adds the third: an opt-in ntfy push notification that fires once when the background scheduler transitions from "no update known" (or "I know about an older release") to "v3.1.0 is available." The notification points the tap-to-open URL at `/updates` so a single tap from your phone lands you on the Apply button. Two-key opt-in: requires both `updates.github.notify.ntfy: true` AND `notifications.events.update_available: true` — the same double-gate every ntfy event uses, with both defaulting to off so existing installs don't suddenly start pushing without explicit consent.

  How it works for users: enable ntfy as you would for any other event type (set `notifications.enabled: true`, point at your ntfy server, set both opt-in flags). The notification body reads "vX.Y.Z is available. You're on vA.B.C. Open the Updates page to apply." with an up-arrow emoji and a tap-to-open that goes straight to `/updates`. The notification fires exactly once per release discovery — not once per poll, not once per page load. If you ignore it for 24 hours, the cooldown allows one more push as a daily nudge until you apply the update or until an even newer version supersedes the available one. If you have multiple notification preferences configured (quiet hours, rate limits, etc.) they all apply to `update_available` the same way they apply to every other event — it's a first-class citizen in the notifier registry, not a special-cased side channel.

  Behind the scenes — the transition-detection logic. The naive implementation would fire on every successful check that finds a newer-than-running tag, which would mean a daily-poll user gets a daily push until they apply (annoying — that's what the gear-badge is for). The right implementation fires only on the *transition*: the moment `last_known_latest` in SQLite changes from one tag to a strictly-newer tag. That requires reading the prior state BEFORE the save, comparing to the new state AFTER the save, and gating the notification on the change. The `_perform_github_check()` function now snapshots `prior_state["last_known_latest"]` inside the `_update_check_lock`, saves the new state, then fires the notification only if (a) the new tag is strictly newer than the prior tag (or prior was NULL — first-discovery counts as a transition), AND (b) the new tag is strictly newer than the running version. Both conditions matter: (a) prevents re-firing every poll until apply, (b) prevents firing for "you discovered v3.1.0 but you're already on v3.1.0" scenarios where a manual install ran ahead of the scheduler. The notifier call itself is wrapped in try/except so a notifier failure can never crash the scheduler — state is saved first, notification is best-effort.

  Behind the scenes — the notifier integration. `update_available` is now in `notifier.py`'s `KNOWN_EVENTS` set and in `NEVER_COOLDOWN` (the per-aircraft cooldown doesn't apply since there's no aircraft context). A new `'updates'` route mapping in `_build_click_url()` translates `click_route="updates"` to `{public_url}/updates` for the tap-to-open — same pattern as `'live'`, `'watchlist'`, `'stats'`, etc. The default cooldown is 1440 minutes (24 hours) in `notifications.cooldown_minutes.update_available`, which is the value most users will leave alone — daily nudge if you've been sleeping on the update, silent otherwise. The notification body uses the `arrow_up` ntfy tag (an upward-pointing emoji) so the notification is visually distinct from the other Aerodrome alerts in your ntfy feed.

  Behind the scenes — the failure modes. If `public_url` isn't set, the tap-to-open falls back to opening the ntfy app instead of `/updates` — the same graceful degradation other events use. If the notifier itself fails (network glitch contacting your ntfy server, malformed config, server down), the state is already saved so the user will still see the update on `/updates` the next time they visit; only the push is lost. If both opt-in flags are true but `notifications.enabled` is false, the notifier's master-switch gate suppresses cleanly. If `update_available` is the only event you've enabled and you've also disabled the master switch, nothing fires — exactly as expected.

  **The v3.0.x arc is now complete.** v3.0.0 made Aerodrome aware of its own next version. v3.0.1 let users apply that version with one click. v3.0.2 means users don't have to look — Aerodrome tells them. Three notification surfaces (gear-menu badge, /updates banner, ntfy push), each independently toggleable; one apply path; one SQLite table holding the cached state; one scheduler thread doing all the periodic work. The channel is dogfooded end-to-end against the real public GitHub Releases API, and the maintainer's own deploys (this one included) flow through it now.

## [3.0.1] — 2026-05-11

### Added
- **Activated the in-app Apply update flow for the GitHub channel — the discover-and-act loop is now complete.** The "GitHub update" card on `/updates`, which v3.0.0 introduced in five visible states, now offers a real **Apply update** button in its update-available state. Click it and Aerodrome downloads the release zip and its `.sha256` from the public asset URLs, verifies the checksum matches, stages the contents into `update/`, and hands off to the existing `POST /api/updates/local/apply` flow — which already handles sudoers drift pre-flight, backup, copy-over, dependency reinstall, and service restart. The apply step itself is unchanged; v3.0.1 is purely a fetch + verify + stage layer in front of infrastructure that already worked. With v3.0.0's check side and v3.0.1's apply side both live, the GitHub channel is now end-to-end self-sufficient — users no longer need to download a zip manually, SCP it to the server, or use the local-upload card to install a discovered update. **Card order on `/updates` flipped to match:** GitHub-update is now position 1, Local update is position 2, ntfy stays third, Changelog stays last. The page intro text dropped its "(soon) GitHub" qualifier (now stale), and the order reflects the new reality where GitHub is the primary update path and local upload is the air-gapped/manual fallback.

  How it works for users: with an update available (i.e. the GitHub card showing `update available` badge), three buttons appear. **Apply update (vX.Y.Z)** is the primary action — click it, confirm the destructive-op prompt (downloads, checksums, backs up, replaces, restarts), and the install transitions through "applying…" → "applied" states with the standard "page will reload in a few seconds" recovery. **View on GitHub** stays as a secondary action for users who want to read the Release notes before committing to apply, useful especially for major versions where the notes might surface migration steps or behavior changes. **Check again** stays available for the user who clicks away and back. If the apply fails partway, the card surfaces the specific error and offers both Try again and Re-check status buttons — failures here include checksum mismatches (refused before staging — never apply an unverified zip), network timeouts during the 60-second per-asset download budget, GitHub returning 404 on an asset (a packaging bug worth reporting), GitHub returning 403 rate-limited (anonymous API limit, suggests waiting an hour), insufficient disk space, and the sudoers-drift case which inherits the existing modal from the local-apply flow automatically.

  Behind the scenes — the reuse architecture. `apply_local_update()` was already source-agnostic about how files got into `update/` — it just looks for `update/VERSION` or `update/aerodrome/VERSION` and proceeds. v3.0.1 doesn't refactor that function or even touch its signature; the new `POST /api/updates/github/apply` endpoint stages the GitHub-sourced content into `update/`, then literally calls `await apply_local_update()` and returns its response unchanged. Everything below the staging layer — sudoers drift check, the `.backups/<timestamp>/` snapshot, the file-by-file copy, the `pip install -r requirements.txt` step, the systemd restart — runs identically whether the user came from local-upload or from the GitHub button. This means the UI's success-rendering can reuse one render path; the success/failure response shape from both endpoints is byte-identical. About 30 lines of UI changes for the apply flow specifically; the rest of the +200 lines in this release is the fetch + verify + stage helpers in server.py.

  Behind the scenes — the staging design choices. The release zip ships with a top-level `aerodrome-vX.Y.Z/` wrapper folder per the v2.98.2 packaging convention, but `apply_local_update()` expects either `update/VERSION` or `update/aerodrome/VERSION` at lookup time. The new `_verify_and_stage_github_zip()` handles this by detecting a common top-level prefix across all zip members and stripping it during extraction — so `aerodrome-v3.0.1/server.py` ends up at `update/server.py`, ready for the apply step. The detection is robust against both wrapped layouts (current) and any future flat layout (no prefix) without code changes. Disk-full during extraction surfaces as a clear "Not enough disk space to stage the update" rather than a half-extracted tree; checksum verification happens BEFORE any disk writes, so a corrupted download never touches `update/` — the trust anchor for the channel runs at the right point in the pipeline. The `update/` directory is wiped clean before extraction (except `UPDATE_README.md` and `.gitkeep` which the local-apply flow's own cleanup preserves), so any previously-staged local upload doesn't bleed into the GitHub apply — the two flows are isolated by design.

  Behind the scenes — the dependency footprint. v3.0.1 adds three stdlib imports inside the endpoint registration scope (`hashlib`, `zipfile`, `io`) — no new package added to `requirements.txt`, consistent with v3.0.0's "use urllib for HTTP, no requests dependency on this path" choice. The 60-second per-asset timeout is twice v3.0.0's 30-second check-API timeout because the assets are larger (5 MB vs a few KB of JSON), but well below any reasonable session-stalling threshold. Apply gates on `updates.github.enabled == true` AND a real update being available (`last_known_latest > running_version` per semver) — a UI cache showing stale "update available" state can't trigger an unwanted apply because the endpoint re-verifies against the server's current SQLite state at request time.

  What's next: **v3.0.2** is the final piece of the v3.0.x arc — adds the optional ntfy push notification for `update_available` events (opt-in like all ntfy event types, requires both `updates.github.notify.ntfy: true` and `notifications.events.update_available: true` to actually fire), plus a polish pass on whatever rough edges surface during dogfooding the now-complete check-and-apply flow.

## [3.0.0] — 2026-05-11

### Added
- **Activated the GitHub-Releases-based update channel — Aerodrome now knows when it's outdated.** Adds a real implementation of `/api/updates/github/check` (previously a stub returning `enabled: false`), a background scheduler that polls GitHub on a configurable interval (default monthly), and a real "GitHub update" card on the `/updates` page replacing the v2.41.3 "coming soon" placeholder. Five distinct UI states render based on cached state: **disabled** (when the master toggle is off in config), **never-checked** (first run, no check has happened yet), **up-to-date** (running version is >= latest GitHub release), **update-available** (a strictly newer release exists), and **error** (last check failed — network timeout, rate limit, etc., with the last known good version still shown alongside). The major version bump is justified because this is the milestone the v3.0.0 slot was reserved for: transitioning from solo-dev private hosting to the public OSS update flow Aerodrome's repository-public state finally enables.

  How it works for users: a new `updates` section in `config.yaml` controls the channel. The defaults — `enabled: true`, `poll_interval: monthly`, `notify.banner: true`, `notify.gear_badge: true` — are the values the launch experience optimizes for: a fresh install does its first check immediately on startup (or on first reaching the `/updates` page), then settles into monthly background polling. Manual "Check now" buttons on the card always work regardless of the polling interval, including when polling is set to "never" (which disables only the *automatic* checks, not the user's ability to ask). The gear-menu amber dot lights up across every admin page when an update is available — the same dot already used for sudoers drift and capacity issues, now with one additional contributor. v3.0.0 is intentionally check-only: the card shows "View on GitHub" for available updates, pointing at the Release page. Actually applying the update still goes through the existing local-update card above the GitHub card (download the zip, upload to update/, click Apply); the in-app "Apply update" button lands in v3.0.1, which is the fetch+verify+stage layer in front of the existing apply infrastructure.

  Behind the scenes — the architecture choices that mattered. The single-row `update_state` table (SQLite migration v8, enforced by a `CHECK (id = 1)` constraint) carries two distinct timestamps: `last_check_ts` (any attempt, success or failure) drives the "last checked: N minutes ago" display, while `last_known_latest_ts` (only successful checks) is what the scheduler's interval-elapsed logic compares against. The separation matters because a transient network glitch shouldn't reset the "we know about v3.0.0" knowledge — only failed checks update the error fields; successful checks update everything. The scheduler is a dedicated daemon thread modeled on `_daily_summary_scheduler` (Aerodrome's established pattern — no central scheduler library, just one thread per periodic task), sleeping at most 1 hour between wakes so config changes propagate quickly without requiring service restart (this is also why `updates` is in `LIVE_KEYS` in `config_validator.py`). A `_update_check_lock` prevents the "Check now" button racing with a scheduled tick, so concurrent manual+automatic checks produce exactly one HTTP call and one state update. Page loads on `/updates` never hit GitHub directly — they read cached state from SQLite via the new endpoint shape, which is what makes the 60-req/hour anonymous rate limit irrelevant at scale (traffic scales with users, polling scales with the configured interval).

  Behind the scenes — defensive bits worth flagging. Version comparison is strict semver-only (`X.Y.Z` parse exactly); non-semver tags including pre-release suffixes (`-rc1`, `+build.42`) are deliberately treated as "no actionable update" rather than risking a misorder comparison. This means pre-releases tagged on GitHub never appear as "available" to existing installs, regardless of whether `releases/latest` ever returns one — belt-and-braces, since `releases/latest` excludes pre-releases anyway. Downgrades are never suggested: when `running > latest_known` (a dev install ahead of the latest tagged release, or a yanked release scenario), the card shows "up to date" rather than offering to drop the user to an older version. Errors during `_get_update_state()` itself (e.g., DB locked momentarily) fall through to the "never checked" state with a logged warning rather than surfacing as a card error — those are infrastructure failures, not GitHub-channel failures. The endpoint accepts `?force=true` for the manual "Check now" path, ignored silently when the master toggle is off (no point hitting GitHub when the feature is disabled). And the scheduler starts unconditionally on boot rather than gated on `enabled: true`, so flipping the toggle on at runtime via config edit takes effect within the next sleep interval (~1 hour worst case) without a service restart — the alternative ("don't start the thread when disabled, require restart to enable") would have made the LIVE_KEYS contract a lie.

  The next two releases close out the v3.0.x arc: v3.0.1 adds the in-app "Apply update" button (fetch zip + sha256 from the Releases asset URLs, verify, stage into `update/`, hand off to the existing `POST /api/updates/local/apply` which already does the right thing source-agnostically), and v3.0.2 adds the optional ntfy push notification (`update_available` event type, opt-in like all ntfy events) plus a polish pass on whatever rough edges surface during dogfooding.
